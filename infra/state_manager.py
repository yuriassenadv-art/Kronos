...
class StateManager:
    ...
    # ── Helpers multi-asset ──────────────────────────────────────────────────
    def list_open_coins(self) -> list[str]:
        """Retorna a lista de coins com posição aberta no momento."""
        return list(self._load().keys())

    def count_open(self) -> int:
        """Quantidade de posições abertas simultâneas."""
        return len(self._load())

    def all_positions(self) -> dict[str, OpenPosition]:
        """Snapshot de todas as posições abertas (coin → OpenPosition)."""
        return {coin: OpenPosition(**raw) for coin, raw in self._load().items()}
